...
exports.get = function getKey(key: Array<string>, getter: () => Promise<any>): Promise<any> {
  ...
  return getter().then(async function (data) {
    const dirname = path.dirname(filename);
    await mkdirpSync(dirname);
    await fs_writeFilePromise(filename, JSON.stringify(data, null, 2));
    return data;
  });
};
/**
...
exports.urlToCacheKey = function (url: string): Array<string> {
  const info = urlParse(url);
  let path = slugify((info.pathname || '/').slice(1).replace(/\//g, '__')) || '__index';
